...
pub fn ebpに即値をセット(n: u32) -> [u8; 5] {
    let buf = n.to_le_bytes();
    [0xbd, buf[0], buf[1], buf[2], buf[3]]
}
...
pub fn rspにoffsetを足したアドレスをrsiに代入(offset: i32) -> Buf {
    fn rspにoffsetを足したアドレスをrsiに代入_i8(offset: i8) -> Buf {
        Buf::from([0x48, 0x8d, 0x74, 0x24, offset.to_le_bytes()[0]])
    }

    fn rspにoffsetを足したアドレスをrsiに代入_i32(offset: i32) -> Buf {
        let buf = offset.to_le_bytes();
        Buf::from([0x48, 0x8d, 0xb4, 0x24, buf[0], buf[1], buf[2], buf[3]])
    }
    if offset >= i8::MIN as i32 && offset <= i8::MAX as i32 {
        rspにoffsetを足したアドレスをrsiに代入_i8(offset as i8)
    } else {
        rspにoffsetを足したアドレスをrsiに代入_i32(offset)
    }
}



pub fn rspにoffsetを足したアドレスをrdiに代入(offset: i32) -> Buf {
    fn rspにoffsetを足したアドレスをrdiに代入_i8(offset: i8) -> Buf {
        Buf::from([0x48, 0x8b, 0x7c, 0x24, offset.to_le_bytes()[0]])
    }

    fn rspにoffsetを足したアドレスをrdiに代入_i32(offset: i32) -> Buf {
        let buf = offset.to_le_bytes();
        Buf::from([0x48, 0x8b, 0xbc, 0x24, buf[0], buf[1], buf[2], buf[3]])
    }
    if offset >= i8::MIN as i32 && offset <= i8::MAX as i32 {
        rspにoffsetを足したアドレスをrdiに代入_i8(offset as i8)
    } else {
        rspにoffsetを足したアドレスをrdiに代入_i32(offset)
    }
}